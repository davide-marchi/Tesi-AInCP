...
# Define the samples size and cluster number
match_W = re.search(r"_W(\d+)", model_folder)
match_K = re.search(r"_K(\d+)", model_folder)
if match_W and match_K and os.path.exists(model_folder + "trained_model"):
    model = jl.load(model_folder + "trained_model")
    sample_size = int(match_W.group(1))
    cluster_number = int(match_K.group(1))
else:
    print("Model, sample size or cluster number not found")
    exit(1)

metadata = pd.read_excel(folder + 'metadata2022_04.xlsx')

clusters = np.zeros((cluster_number), dtype=int)
