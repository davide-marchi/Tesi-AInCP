...
from diff_create_windows import diff_create_windows
from diff_save_model_stats import diff_save_model_stats
...
def diff_train_clustering_model(folder, plot_clusters, patients, model_type, WINDOW_SIZE, N_CLUSTERS, INIT_ALGORITHM, METRIC):
    ...
    series, y_AHA, y_MACS, total, taken, lost = diff_create_windows(WINDOW_SIZE, folder, patients)

    # Create a DataFrame with a single column of Series objects
    X = pd.DataFrame({'series': series})

    model_folder = "./Blocco 1/difference_version/" + str(patients) + "_patients/" + model_type + "/" + model_name
    ...
    if not os.path.exists(model_folder + "/trained_model"):
        ...
        diff_save_model_stats(X, y_AHA, y_MACS, y_pred, model, model_name, N_CLUSTERS, model_folder)
